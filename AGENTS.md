...
- Scope: This file governs the whole repo tree. For reuse across future projects/repos, see "Appendix I — Multi‑Repo Adoption & Sync".
...
## Appendix I — Multi‑Repo Adoption & Sync

- Single‑repo scope: AGENTS.md applies to the entire directory tree rooted where it lives.
- New repos (recommended): Mark this repo as a GitHub Template and create new repos from it so they inherit AGENTS.md, `.agents.yml`, and convention scaffolding.
  - Example: `gh repo edit <owner>/<repo> --template=true` then `gh repo create <owner>/<new> --template <owner>/<repo>`
- Existing or ad‑hoc projects: use the bootstrap script to seed/update only the policy files.
  - Run: `scripts/seed_agents.sh <target_dir>` (non‑destructive; add `-f` to overwrite)
- Git init templates (optional, advanced): set a global template dir to auto‑copy policy files into every `git init`.
  - `git config --global init.templateDir ~/.config/git/template`
  - Place `AGENTS.md` and `.agents.yml` in `~/.config/git/template/`
- Org‑level guardrails (optional): add a lightweight CI check that verifies presence of `AGENTS.md` and `.agents.yml` and runs `make check` → `make all` per Appendix A.
- Versioning & updates: treat AGENTS.md as living policy. For existing repos, re‑run the bootstrap script to sync changes or open a PR that updates only policy files.
